    ...
    public static void main(String[] args) throws ReflectiveOperationException, IOException {
        ...
        for (File parent:root.listFiles()) {
            for (File hack:parent.listFiles()) {
                ...
                Path original = hack.toPath().resolve("romhack-original");
                ...
                String newName = currentName.substring(0, indexOfEnd).trim() + expectedFolderNamePostfix + "." + extension;
                if (Files.exists(original.resolve("1-sources"))) {
                    System.out.println(original);
                }

                if (!currentName.equals(newName)) {
                    File newHack = parent.toPath().resolve(newName).toFile();
                    Path source = original.resolve("romhack-original");
                    //Files.move(hack.toPath(), newHack.toPath());
